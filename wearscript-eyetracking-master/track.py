...
PARAMS = {'_delta':7, '_min_area': 2000, '_max_area': 20000, '_max_variation': .25, '_min_diversity': .2, '_max_evolution': 200, '_area_threshold': 1.01, '_min_margin': .003, '_edge_blur_size': 5, 'pupil_intensity': 120, 'pupil_ratio': 3.0, 'save_data': False}
...
def debug_iter(box, frame, hull, timestamp,extra):
    ...
    if key == -1:
        ...
        print ('Got new Intensity[%f]' % PARAMS['pupil_intensity'])
    elif key == 118: # v
        PARAMS['pupil_intensity'] -= 5
    elif key == 115: # s
    ...
    def callback(ws, **kw):
        ...
        while run[0] != 'QUIT':
            for box, frame, hull, timestamp, extra in pupil_iter(**kw):
                if kw.get('debug'):
                    run[0] = debug_iter(box, frame, hull, timestamp,extra)# added extra as a parameter  5/25
                    
                if ((run[0]!= 'RELOAD') and (run[0] != None)):#changed this to not to break on reload
                    break
                ...
                if box is None:
                    continue
                ws.publish('sensors:eyetracker', {'Pupil Eyetracker': -2}, {'Pupil Eyetracker': [[[box[0][1], box[0][0], extra['ratio'], extra['area'], extra['radius'], PARAMS['save_data']], time.time(), int(time.time() * 1000000000)]]})
                time.sleep(1)
